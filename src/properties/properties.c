...
PetscReal SaltWaterMassDiffusivity(PetscReal temperature, PetscReal salinity)
{
    ...
    PetscReal density, molarity, molar_mass_NaCl, diffusivity;

    density = SaltWaterDensity(temperature, 0.0);
    //density_NaCl = 2160.0;
    molar_mass_NaCl = 58.44e-3;
    //molarity = (1.0 / molar_mass_NaCl) * salinity / ((1.0-salinity) / density + salinity / density_NaCl); // tenho que mudar essa expressão
    /* bulk salt molarity C_b = w_b * rho / M_s  (mol/m^3) */
    molarity = salinity * density / molar_mass_NaCl;
    molarity /= 1000.0;
...
PetscErrorCode SaltWaterPropBuild(SaltWaterProperties *salt_water_prop, PetscReal temperature, PetscReal salinity)
{
    ...
    PetscReal density, specific_heat, dyn_viscosity, thermal_conductivity, vapor_pressure, latent_heat_vaporization, mass_diffusivity;
...
Reference: Cengel, Y. and Boles, M. (2015) Thermodynamics: An Engineering Approach. 8th Edition, McGraw-Hill.
*/
...
PetscReal DryAirSpecificHeat(PetscReal temperature)
{
    ...
    specific_heat = (1.0 / air_molar_mass) * (a[0] 
                                          + a[1] * (temperature) 
                                          + a[2] * PetscPowRealInt(temperature, 2) 
                                          + a[3] * PetscPowRealInt(temperature, 3));
