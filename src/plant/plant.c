...
PetscErrorCode PlantBalances(TS ts, PetscReal time, Vec x, Vec x_t, Vec f, void *ctx)
{
    SolverCtx *solver_ctx = (SolverCtx *)ctx;
    EntryData entry_data = solver_ctx->entry_data;
    DessalData dessal_data = entry_data.dessal_data;
    DM da = solver_ctx->da;
    PetscScalar *x_array, *x_t_array, *f_array;
    Vec x_local;

    DMGetLocalVector(da, &x_local);
    DMGlobalToLocal(da, x, INSERT_VALUES, x_local);
    DMDAVecGetArray(da, x_local, &x_array);
    DMDAVecGetArray(da, x_t, &x_t_array);
...
PetscErrorCode RunPlant(EntryData *entry_data)
{
    ...
    Vec solution;
    Mat jac;
    char file[64] = "./results/entry_data.csv";
    ...
    ExportEntryData(entry_data, file);

    InitialGuess(solution, &solver_ctx);

    TSSetSolution(solver_ctx.ts, solution);
    TSSetIFunction(solver_ctx.ts, NULL, PlantBalances, (void *)&solver_ctx);
    TSMonitorSet(solver_ctx.ts, StepMonitor, (void *)&solver_ctx, NULL);
    TSGetSNES(solver_ctx.ts, &snes);
    SNESSetJacobian(snes, jac, jac, SNESComputeJacobianDefault, NULL);

    TSSolve(solver_ctx.ts, solution);
